...
In an ideal world, the result of running a program in this language would be a delicious drink. However, since I don't have the resources to procure an actual automated coffee machine to program using this lanugauge, I will have to come up with some alternative "final product" of the language. I'm currently leaning towards building an API that represents the coffee machine. API calls would result in some printed statement like "pulling" + n + "shots of espresso now". Thus, the final product of a program will just be a bunch of printed statements, but the idea is that if you swapped out the print statments for actual implementation of the automated coffee making, you could keep the API the same and the code that the user writes to create the drink wouldn't have to be changed at all.
